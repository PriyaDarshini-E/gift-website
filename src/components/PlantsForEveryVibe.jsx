...
import { ChevronLeft, ChevronRight, Flame, ArrowRight, Eye, ShoppingBag } from 'lucide-react';
...
export default function PlantsForEveryVibe({ title = "Vacuum Insulated Flasks & Bottles", subtitle = "Keep beverages piping hot or icy cold all day long — high grade 304/316 stainless steel", categorySlug = "vacuum-insulated" }) {
  ...
  // Dynamically filter products related to bottles/flasks
  const bottleProducts = rawProducts.filter(p => {
    const name = (p.name || '').toLowerCase();
    const cat = (p.category || '').toLowerCase();
    return name.includes('flask') || name.includes('bottle') || name.includes('vacuum') || name.includes('hydra') || name.includes('trek') || cat.includes('vacuum') || cat.includes('bottle');
  }).slice(0, 15);

  const displayList = bottleProducts.length > 0 ? bottleProducts : rawProducts.slice(20, 35);
  ...
  return (
    <div className="bg-[#f2f8f6] py-14 border-b border-emerald-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8">
          ...
          <Link 
            ...
            className="hidden sm:inline-flex items-center gap-1 text-sm font-bold text-emerald-900 hover:text-emerald-700 transition-colors group/header"
          >
            Explore All Flasks
        ...
        <div className="relative" ref={containerRef}>
          ...
          <button
            ...
            className={`absolute -left-4 top-[45%] -translate-y-1/2 z-20 w-10 h-10 rounded-full
              bg-white shadow-lg border border-emerald-200 flex items-center justify-center
              text-gray-700 hover:text-emerald-900 transition-all duration-200 cursor-pointer
          ...
          <div className="overflow-hidden py-2">
            ...
            >
              {displayList.map((item) => (
                ...
                >
                  <div className="bg-white rounded-2xl p-3 shadow-xs hover:shadow-md transition-all duration-300 border border-emerald-100 flex flex-col justify-between h-full">
                    
                    {/* Image Area */}
                    <div 
                      onClick={() => openProductModal(item)}
                      className="w-full aspect-square bg-emerald-50/50 rounded-xl overflow-hidden flex items-center justify-center relative cursor-pointer"
                    >
                      <FixedImage
                      ...
                      {item.brand && (
                        <span className="absolute top-2 left-2 px-2 py-0.5 bg-emerald-950/80 backdrop-blur-xs text-white text-[9px] font-bold rounded-full uppercase tracking-wider">
                          {item.brand}
                        </span>
                      )}
                    ...
                    <div className="pt-3 flex flex-col justify-between flex-1">
                      <p 
                        onClick={() => openProductModal(item)}
                        className="text-[13px] font-semibold text-gray-800 font-sans line-clamp-2 hover:text-emerald-700 transition-colors cursor-pointer mb-1.5"
                      >
                        {item.name}
                      </p>
                      ...
                      <div>
                        ...
                        <button
                          ...
                          className="w-full py-2 bg-emerald-50 hover:bg-emerald-700 text-emerald-900 hover:text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                        >
